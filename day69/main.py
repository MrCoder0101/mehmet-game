...
from forms import CreatePostForm, RegisterForm, LoginForm, CommetForm
    ...
    if form.validate_on_submit():
        ...
        try:
            new_user = User(email=data_email,
                            password=generate_password_hash(data_password, method="pbkdf2:sha256", salt_length=8),
                            name=data_name)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        try:
            user = db.session.query(User).filter_by(email=data_email).first()
            if not user:
                flash("That email does not exist, please try again.")
                return redirect(url_for('login'))
            elif not check_password_hash(user.password, data_password):
...
def show_post(post_id):
    ...
    if comment_form.validate_on_submit():
        ...
        try:
            new_comment = Comment(text=comment_data, comment_author=current_user, parent_post=requested_post)
    ...
    if form.validate_on_submit():
        try:
            new_post = BlogPost(
                ...
                body=form.body.data,
                img_url=form.img_url.data, author=current_user)
